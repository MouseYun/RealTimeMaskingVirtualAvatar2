...
public class ARFaceBlendShapeVisualizer : MonoBehaviour
{
    ...
    private Renderer[] _characterRenderers; // 화면 밖에 있을 때 모든 랜더러 비활성화 하기 위한 선언;
    ...
    private const int BlendShapeIndexLeftEyeBlink = 0;
    private const int BlendShapeIndexRightEyeBlink = 7;

    private const int BlendShapeIndexMouthOpen = 17;
    private const int BlendShapeIndexMouthU = 20;
    ...
    private void Start()
    {
        ...
        _arFace.updated += OnFaceUpdated; // ARFace정보가 업데이트 될 때마다 OnFaceUpdated() 실행.
    ...
    private void ApplyMouse()
    {
        ...
        faceMeshRenderer.SetBlendShapeWeight(BlendShapeIndexMouthOpen, mouthOpenValue);

        var mouthUValue = (_arKitBlendShapeValueTable[ARKitBlendShapeLocation.MouthPucker]);

        faceMeshRenderer.SetBlendShapeWeight(BlendShapeIndexMouthU, mouthUValue);
    }
